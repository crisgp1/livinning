...
const createPropertyLogger = () => {
  if (isServer) {
    ...
    try {
      ...
      // Define custom log levels for property operations
      ...
      // Custom format for property operations
      const propertyFormat = winston.format.combine(
        winston.format.timestamp({
          format: 'YYYY-MM-DD HH:mm:ss.SSS'
        }),
        winston.format.errors({ stack: true }),
        winston.format.metadata({
          fillExcept: ['message', 'level', 'timestamp', 'propertyId', 'userId', 'action']
        }),
        winston.format.printf(({ level, message, timestamp, propertyId, userId, action, metadata }: any) => {
      ...
      return winston.createLogger({
        ...
        level: process.env.NODE_ENV === 'production' ? 'info' : 'property_create',
        format: propertyFormat,
        defaultMeta: {
          service: 'property-frontend',
          component: 'server'
        },
        transports: [
          ...
          // File transport for property creation operations
          new winston.transports.File({
            filename: 'logs/property-creation.log',
            level: 'property_create',
            maxsize: 5242880, // 5MB
            maxFiles: 5,
            format: propertyFormat
          }),

          // File transport for property editing operations
          new winston.transports.File({
            filename: 'logs/property-editing.log',
            level: 'property_edit',
            maxsize: 5242880, // 5MB
            maxFiles: 5,
            format: propertyFormat
          ...
          // File transport for property viewing operations
          new winston.transports.File({
            filename: 'logs/property-viewing.log',
            level: 'property_view',
            maxsize: 5242880, // 5MB
            maxFiles: 5,
            format: propertyFormat
          ...
          // General property actions log
          new winston.transports.File({
            filename: 'logs/property-actions.log',
            level: 'property_action',
            maxsize: 10485760, // 10MB
            maxFiles: 10,
            format: propertyFormat
          }),

          // Error log for all property operations
          new winston.transports.File({
            filename: 'logs/property-errors.log',
            level: 'error',
            maxsize: 5242880, // 5MB
            maxFiles: 5,
            format: propertyFormat
        ...
        exceptionHandlers: [
          new winston.transports.File({ filename: 'logs/property-exceptions.log' })
        ],

        rejectionHandlers: [
          new winston.transports.File({ filename: 'logs/property-rejections.log' })
        ]
      });
    } catch (error) {
